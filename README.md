...
This repo holds the Ethereum smart contracts for [BASΞD VITALIK](https://basedvitalik.io).

It is a simple ERC721 contract using the fancy new [ERC721A standard](https://www.erc721a.org/) in addition to some OpenZeppelin components.
...
At this point, you've got a deployed contract which will mint new Vitaliks to those who invoke the `mintVitaliks` function and pay for the gas required. How do you actually get someone to do this? You need some Javascript on a website to actually do that contract interaction.

Here's what you need to gather:
* Contract address (outputted in `truffle deploy` step above)
* Contract ABI (`cat build/contracts/BasedVitalik.json | jq .abi` - assumes you have `jq` installed to your system)
...
See [BASΞD VITALIK](https://basedvitalik.io/js/mint.js) for an example.
...
truffle(rinkeby)> let c = await BasedVitalik.deployed();
undefined
